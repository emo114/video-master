<template>
  <div>
    <Sticky>
      <x-header style="background-color: #FFFFFF;" :left-options="{backText: ''}"><span style="color: black">历史记录
     </span><a slot="right" @click="showSelect">
        <span style="color: #DBB97C;"> {{menu}}</span>
      </a>
      </x-header>
    </Sticky>
    <flexbox :gutter="0" wrap="wrap" v-for="(movie,index) in movies" :key="index">
      <flexbox-item v-show="isSelect" :span="leftPer">
        <div class="flex-demo">
          <div class="mui-checkbox-con">
            <label>
              <input class="mui-checkbox checkbox-orange" type="checkbox" @click="selectDelItem(movie.id)"
                     :checked="myAllSelect" :disabled="myAllSelect"></label>
          </div>
        </div>
      </flexbox-item>
      <flexbox-item :span="midPer">
        <div class="flex-demo">
    ...
    <div class="footerView" v-show="isSelect">
  ...
  export default {
    ...
    methods: {
      ...
      async deleteSelected() { //点击删除
        ...
        const result = await deleteViewHistory(params);
        if (result.retCode === '1') {
      ...
      async getMemberViewHistoryMore(params) {
        this.$vux.loading.show({
          text: '加载中'
        });
        const result = await getMemberViewHistoryMore(params);
        if (result.retCode === '1') {
